...
def mock_ny_times_data():
    ...
    return pd.DataFrame({
        'section_name': ['Business Day', 'Business Day', 'Politics', 'Business Day'],
        'snippet': ['Economic news 1', 'Stock market up', 'Political debate', 'Tech company profits'],
        'lead_paragraph': ['Details about economy', 'Stock details', 'Political details', 'Tech details'],
        'pub_date': ['2023-01-01T12:00:00Z', '2023-01-02T12:00:00Z', '2023-01-03T12:00:00Z', '2023-01-04T12:00:00Z']
    })
...
def mock_business_data():
    ...
    return pd.DataFrame({
        'text': ['Economic news 1_Details about economy', 'Stock market up_Stock details', 
...
def mock_sentiment_data():
    ...
    return pd.DataFrame({
        'label': ['Positive', 'Very Positive', 'Neutral', 'Positive', 'Very Positive'],
...
def mock_pivot_data():
    ...
    df = pd.DataFrame(index=['2023-01-01', '2023-01-02', '2023-01-03'], 
...
def test_preprocess_sentiment_scores(mock_sentiment_data):
    ...
    assert all(label in result.columns for label in 
...
def test_train_and_predict(monkeypatch):
    ...
    class MockProphet:
        ...
        def make_future_dataframe(self, periods):
            return pd.DataFrame({'ds': pd.date_range(start='2023-01-01', periods=10+periods)})
